...
abstract public class Search {
    ...
    public boolean searchTypeIsTrue(Song song, Album album) {
        if (searchType == null) {
            return false;
        } else if (searchType == Criteria.ARTIST) {
            return artistSearch(song, album);
        } else if (searchType == Criteria.GENRE) {
            return genreSearch(song, album);
        } else if (searchType == Criteria.RANDOM) {
            return randomSearch();
        } else {
            return false;
        }
    ...
    private boolean artistSearch(Song song, Album album) {
        return !album.songs.isEmpty() && !Collections.disjoint(song.artistNames, album.songs.get(0).getArtistNames());
    }

    private boolean genreSearch(Song song, Album album) {
         return !album.songs.isEmpty() && !Collections.disjoint(song.genres, album.songs.get(0).genres);
    }
